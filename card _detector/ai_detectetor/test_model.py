import os
import torch
import torchvision.transforms as transforms
from torchvision import models
from PIL import Image
from matplotlib import pyplot as plt

# Définir le chemin du modèle et du dossier de test
MODEL_PATH = r"C:\Users\conta\Documents\python\model_efficientnet.pth"
TEST_DIR = r"C:\Users\conta\Desktop\test image"
DEVICE = torch.device("cuda" if torch.cuda.is_available() else "cpu")

# Définir les classes
CLASSES = [
    "10C", "10D", "10H", "10S", "2C", "2D", "2H", "2S", "3C", "3D", "3H", "3S",
    "4C", "4D", "4H", "4S", "5C", "5D", "5H", "5S", "6C", "6D", "6H", "6S",
    "7C", "7D", "7H", "7S", "8C", "8D", "8H", "8S", "9C", "9D", "9H", "9S",
    "AC", "AD", "AH", "AS", "JC", "JD", "JH", "JS", "KC", "KD", "KH", "KS",
    "QC", "QD", "QH", "QS", "cartes_combinees", "cartes_combinees_retournees",
    "cartes_retournees", "non_cartes"
]


# Charger le modèle
def load_model(model_path, num_classes):
    print("Chargement du modèle EfficientNet...")
    model = models.efficientnet_b0(pretrained=False)
    model.classifier[1] = torch.nn.Linear(model.classifier[1].in_features, num_classes)
    model.load_state_dict(torch.load(model_path, map_location=DEVICE), strict=True)
    model.to(DEVICE)
    model.eval()
    return model


# Transformer les images
transform = transforms.Compose([
    transforms.Resize((224, 224)),
    transforms.ToTensor(),
    transforms.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])
])


# Tester le modèle
def test_model(model, test_dir):
    if not os.path.exists(test_dir):
        print(f"Le dossier de test n'existe pas : {test_dir}")
        return

    print(f"Test des images dans le dossier : {test_dir}")
    for img_file in os.listdir(test_dir):
        img_path = os.path.join(test_dir, img_file)
        if os.path.isfile(img_path):
            try:
                # Charger l'image
                img = Image.open(img_path).convert("RGB")
                input_tensor = transform(img).unsqueeze(0).to(DEVICE)

                # Faire la prédiction
                with torch.no_grad():
                    output = model(input_tensor)
                    _, predicted = torch.max(output, 1)
                    predicted_class = CLASSES[predicted.item()]

                # Afficher l'image avec la prédiction
                plt.imshow(img)
                plt.title(f"Prédiction : {predicted_class}")
                plt.axis("off")
                plt.show()

            except Exception as e:
                print(f"Erreur avec l'image {img_file} : {e}")


# Exécuter le script
if __name__ == "__main__":
    model = load_model(MODEL_PATH, len(CLASSES))
    test_model(model, TEST_DIR)
